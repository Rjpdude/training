...
def main(
    pretrained_path: str = typer.Option(
        default="NousResearch/Nous-Hermes-2-Mixtral-8x7B-SFT"
    ),
    ...
    config = BitsAndBytesConfig(
        load_in_4bit=True, load_in_8bit=False, bnb_4bit_use_double_quant=True
    )
    model = MixtralForCausalLM.from_pretrained(
        pretrained_path, device_map="auto", quantization_config=config
    )
   ...
   },
   {
            ...
            [
                {
                    ...
                    f"conversations context. Here is an example format: {tools}",
                },
    ...
    def preprocess(convoset):
        conversations = []
        for convo in convosets["conversations"]:
            def convert(convo):
                convo = {
                    "messages": [
            ...
            conversations.append([
                dict(messages=convert(conversations))
                for conversations in convoset["conversations"]
            ])
